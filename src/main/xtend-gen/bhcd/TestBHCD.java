...
public class TestBHCD {
  ...
  public void invariance() {
    ...
    final BHCD model = new BHCD.Builder().setGraph(graph).setDend(BasicDendrogram.initDend(graph)).setAlpha(alpha).setBeta(beta).build();
    ...
    final Instance<BHCD> instance = new Instance<BHCD>(model, _function, _function_1, _function_2, _function_3);
    ExactInvarianceTest _exactInvarianceTest = new ExactInvarianceTest();
    final Procedure1<ExactInvarianceTest> _function_4 = (ExactInvarianceTest it) -> {
      it.add(instance);
    };
    final ExactInvarianceTest test = ObjectExtensions.<ExactInvarianceTest>operator_doubleArrow(_exactInvarianceTest, _function_4);
