##############################################################################
# Summary Stats for Terrorist Network & Simulations
#
# 
##############################################################################
library(igraph)
...
tadj <- read.csv("data/metabolic/links_tpa_reduced.csv", header = FALSE)
...
tadj <- tadj + 1 # readjust numbering if starting from zero

tgraph <- graph_from_edgelist(tadj, directed=FALSE)

# remove duplicate edges
which(which_multiple(tgraph))
...
# edge count
ecount(tgraph) # 152 # 113 # 1255
vcount(tgraph) # 62 # 75 # 524

# find connected components
no.clusters(tgraph)
c1 <- clusters(tgraph)
nS <- which(c1$csize > 1) # find all clusters with more than one node
g <- induced_subgraph(tgraph, which(c1$membership %in% nS)) 
ecount(g)
vcount(g)
(stats <- data.frame(edg_dens=edge_density(g), cc=transitivity(g, type="global"), 
											 diam=diameter(g, directed=F), avg_dist=mean_distance(g, directed=F), 
											 avg_deg=mean(degree(g, mode="all"))))

# create a dataframe to store stats
(t_stats <- data.frame(edg_dens=edge_density(tgraph), cc=transitivity(tgraph, type="global"), 
										 diam=diameter(tgraph, directed=F), avg_dist=mean_distance(tgraph, directed=F), 
										 avg_deg=mean(degree(tgraph, mode="all"))))


# plot the degree dist'n

# get the vertex degrees
tdeg <- data.frame(deg=degree(tgraph, mode='all'))

(p1 <- ggplot(tdeg, aes(x=deg)) + geom_histogram(aes(y=..density..), color="darkblue", fill="lightblue", binwidth = 1) +
	ggtitle("Histogram of Vertex Degree for the Terrorist Network") + ylab("fraction of vertices") +
	xlab("degree"))

# culmulative degree distribution
deg.dist <- degree_distribution(tgraph, cumulative=T, mode="all")
plot( x=0:max(tdeg), y=1-deg.dist, pch=19, cex=1.2, col="orange", xlab="Degree", ylab="Cumulative Frequency")
