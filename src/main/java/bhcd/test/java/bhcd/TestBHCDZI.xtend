...
class TestBHCDZI {
  ...
  def void invariance() {
    ...
    val model = (new BHCD.Builder).setGraph(graph).setDend(BasicDendrogram::initDend(graph)).setAlpha(alpha).setBeta(beta).setTheta(theta).build
    ...
    val instance = new Instance(
      ...
      [it.beta.doubleValue],
      [it.theta.doubleValue],
      [(graph.getNumEdges / ((graph.getNumVertices)*(graph.getNumVertices - 1) / 2)).doubleValue] // edge density
   	)
