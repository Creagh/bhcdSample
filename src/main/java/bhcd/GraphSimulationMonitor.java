...
public class GraphSimulationMonitor implements TidilySerializable {
  ...
  final RealVar theta; // mixture parameter for ZI model
  final Random random = new Random(1);
  final BasicDendrogram dendogram;
  final int visualizationThinning = 1000;
  static int iter = 0;
  
  public GraphSimulationMonitor(int [] vertices, RealVar alpha, RealVar beta, RealVar theta, BasicDendrogram dendogram) {
  ...
  public void serialize(Context context) {
    simulatedGraph.sampleGraph(random, alpha.doubleValue(), beta.doubleValue(), theta.doubleValue(), dendogram);
